...
const skills = [
  ...
  // Mobile
  { name: "iOS Development", level: 80, category: "mobile", icon: "ios" },
  { name: "Swift", level: 75, category: "mobile", icon: "swift" },
  { name: "React Native", level: 85, category: "mobile", icon: "react" },

  // Tools
  { name: "Git", level: 90, category: "tools", icon: "git" },
  { name: "GitHub", level: 90, category: "tools", icon: "github" },
  { name: "VS Code", level: 95, category: "tools", icon: "vscode" },
  { name: "C++", level: 70, category: "tools", icon: "java" }
];
...
const categories = [
  ...
  { id: "mobile", label: "Mobile", color: "bg-gradient-to-r from-orange-500 to-yellow-500" },
  { id: "tools", label: "Tools", color: "bg-gradient-to-r from-red-500 to-rose-500" },
];
...
const InfiniteScrollSkills = ({ skills }) => {
  ...
  return (
    <div className="overflow-hidden py-4 sm:py-8">
      <motion.div
        className="flex gap-4 sm:gap-8 mb-4 sm:mb-8 px-4 sm:px-8"
        animate={{ x: ["0%", "-100%"] }}
        transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
      >
        {duplicatedSkills.map((skill, index) => (
          <div key={`${skill.name}-${index}`} className="flex-shrink-0 flex flex-col items-center gap-1 sm:gap-2">
      ...
      >
        {[...duplicatedSkills].reverse().map((skill, index) => (
          <div key={`${skill.name}-reverse-${index}`} className="flex-shrink-0 flex flex-col items-center gap-1 sm:gap-2">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-card border-2 border-primary/50 flex items-center justify-center shadow-lg hover:scale-110 transition-transform">
              <img src={iconImages[skill.icon]} alt={skill.name} className="w-6 h-6 sm:w-8 sm:h-8 object-contain" />
            </div>
            <span className="text-xs sm:text-sm font-medium text-center">{skill.name}</span>
...
export const SkillsSection = () => {
  ...
  return (
    <section id="skills" className="py-20 md:py-20 px-4 bg-gradient-to-br from-background via-secondary/5 to-background">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16">
          {categories.map((category) => (
            <motion.button
              ...
              className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-full font-medium text-sm sm:text-base border border-transparent hover:shadow-lg ${
            ...
            <AnimatePresence mode="popLayout">
              {filteredSkills.map((skill) => (
                <motion.div
                  ...
                  className="bg-card p-4 sm:p-6 rounded-2xl border border-border/30 hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-lg group"
                >
                  <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-5">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-card border-2 border-primary/50 flex items-center justify-center">
                      <img src={iconImages[skill.icon]} alt={skill.name} className="w-5 h-5 sm:w-6 sm:h-6 object-contain" />
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="mt-1 sm:mt-2 flex justify-between text-xs text-muted-foreground">
                        <span>Basic</span>
                        <span>Advanced</span>
                        <span>Expert</span>
                      </div>
